...
##### Interpreting a linear clasifier
...
- $f(x_i,W,b)=Wx_i+b$ $-> $ $f(xi,W)=Wxif(x_i,W)=Wx_i$
...
- $Li=∑_{j≠yi} max( 0, s_{j}−s_{yi}+Δ )$
- In linear classifier: $Li=∑_{j≠yi} max( 0, w^T_{j}x_i−w^T_{y_i}x_i+Δ )$, $w_i$ is the i_th row of W reshaped as a column
...
- We can see
  ...
  as the normalized probability assigned to the correc label $y_i$ given the image $x_i$
